...
const handleAddToCart = async () => {
    try {
        ...
        const carritosResponse = await axios.get(`http://192.168.1.16/transacciones/carritos/`, {
            params: { usuario: USER_ID }
        });

        // Si hay carritos, usa el primero (o ajusta según la lógica deseada)
        if (carritosResponse.data.length > 0) {
            carritoId = carritosResponse.data[0].id;
        }

        if (!carritoId) {
            // Si no hay carrito existente, crea uno nuevo sin productos_detalle
            const newCarritoResponse = await axios.post(`http://192.168.1.16/transacciones/carritos/`, {
                usuario: USER_ID
            });
            carritoId = newCarritoResponse.data.id; // Ahora tenemos el ID del nuevo carrito
        }

        // Ahora, añadir un nuevo producto al carrito
        await axios.post(`http://192.168.1.16/transacciones/carrito_detalle/`, {
            carrito: carritoId,
...
useEffect(() => {
        const fetchProductDetails = async () => {
            try {
                const response = await axios.get(`http://192.168.1.16/productos/productos/${productId}/`);
        ...
        fetchProductDetails();
         const fetchProductVariants = async () => {
            try {
                const response = await axios.get('http://192.168.1.16/productos/productosdetalle/');
